...
class Stepwise_regression_selector:
    ...
    def _finalize_display_scores(self):
        """Convert max-oriented internal scores to user-facing metric values."""
        if self.subsets is None:
            return

        for subset in self.subsets:
            if self.inner_crit == "RSS":
                # Internally RSS is maximized as -RSS; display the original RSS scale.
                subset["inner_score"] = float(-subset["inner_score"])

            if self.outer_crit in {"AIC", "BIC", "Cp", "cv"}:
                # Internally minimized metrics are maximized via sign flip.
                # For cv this converts neg-MSE back to MSE.
                subset["outer_score"] = float(-subset["outer_score"])
